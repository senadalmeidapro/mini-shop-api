import type { NextFunction, Request, Response } from 'express';
import { AddressService } from '../services/address.service.ts';

export class AddressController {
  private readonly address: AddressService;

  constructor() {
    this.address = new AddressService();
  }

  async create(req: Request, res: Response, next: NextFunction) {
    try {
      const address = await this.address.createAddress(req.validated.body);

      return res.status(201).json(address);
    } catch (error) {
      next(error);
    }
  }

  async createMany(req: Request, res: Response, next: NextFunction) {
    try {
      const address = await this.address.createAddresss(req.validated.body);

      return res.status(201).json(address);
    } catch (error) {
      next(error);
    }
  }

  async get(req: Request, res: Response, next: NextFunction) {
    try {
      const id = Number(req.validated.params.id);

      const address = await this.address.getAddress(id);

      if (!address) {
        return res.status(404).json({
          message: 'Address not found',
        });
      }

      return res.status(200).json(address);
    } catch (error) {
      next(error);
    }
  }

  async getMany(req: Request, res: Response, next: NextFunction) {
    try {
      const { page, limit, ...data } = req.validated.query;

      const query = {
        data,
        page: Number(page),
        take: Number(limit),
      };

      const addresss = await this.address.getAddresss(query);

      return res.status(200).json(addresss);
    } catch (error) {
      next(error);
    }
  }

  async update(req: Request, res: Response, next: NextFunction) {
    try {
      const id = Number(req.validated.params.id);
      const data = req.validated.body;

      const address = await this.address.updateAddress(id, data);

      if (!address) {
        return res.status(400).json({
          message: 'Address not found',
        });
      }

      return res.status(200).json(address);
    } catch (error) {
      next(error);
    }
  }

  async updateMany(req: Request, res: Response, next: NextFunction) {
    try {
      const ids = req.validated.body.ids as number[];
      const data = req.validated.body.data;

      const addresss = await this.address.updateAddresss(ids, data);

      return res.status(200).json(addresss);
    } catch (error) {
      next(error);
    }
  }

  async delete(req: Request, res: Response, next: NextFunction) {
    try {
      const id = Number(req.validated.params.id);

      const result = await this.address.deleteAddress(id);

      if (!result) {
        return res.status(400).json({ message: 'Address not found' });
      }
    } catch (error) {
      next(error);
    }
  }

  async deleteMany(req: Request, res: Response, next: NextFunction) {
    try {
      const ids = req.validated.body.ids as number[];

      const result = await this.address.deleteAddresss(ids);

      if (!result) {
        return res.status(400).json({ message: 'Address not found' });
      }
    } catch (error) {
      next(error);
    }
  }
}
